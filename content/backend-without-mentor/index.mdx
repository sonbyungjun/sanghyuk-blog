...
title: 사수없이 백엔드 구성하기
description: 신입 개발자가 사수 없이 DB 설계부터 백엔드 아키텍처까지 혼자 해낸 경험담
createdAt: "2020/01/15"
updatedAt: ""
tags: ["백엔드", "Node.js", "신입", "경험담"]
featured: true
---
...
{/* [이미지 1: 빈 사무실에서 혼자 모니터를 바라보는 신입 개발자의 뒷모습] */}
...
{/* [이미지 2: 노트북 화면에 Node.js 강의 영상이 재생되고, 옆에 필기 노트가 있는 모습] */}
...
📦 내가 구성한 프로젝트 구조
├── src/
│   ├── controllers/    # 공통 컨트롤러 로직
│   ├── routes/         # 도메인별 라우트 (API 엔드포인트)
│   │   ├── board/
│   │   ├── order/
│   │   ├── shop/
│   │   ├── payment/
...
{/* [이미지 3: 왼쪽에 Java Spring 구조, 오른쪽에 Node.js 구조가 화살표로 매핑된 비교 다이어그램] */}

### 왜 이 구조가 좋았나
- **역할 분리**: 각 파일이 하나의 책임만 가짐
- **도메인별 정리**: routes 안에서 기능별로 폴더 분리
- **유지보수**: 나중에 코드 찾기 쉬움
...
{/* [이미지 4: dbdiagram.io에서 ERD를 그리고 있는 화면 캡처 스타일 일러스트] */}
...
{/* [이미지 5: 복잡한 마이크로서비스 구조(X 표시)와 단순한 모놀리식 구조(O 표시) 비교] */}

### 도메인별 폴더 분리

```javascript
// routes 폴더 안에서 도메인별로 분리
src/routes/
├── board/           # 게시판
├── boardComment/    # 게시판 댓글
├── order/           # 주문
...
{/* [이미지 6: 구글 검색창에 에러 메시지를 붙여넣기 하는 모습] */}
...
{/* [이미지 7: AWS 콘솔 화면과 터미널이 나란히 있는 배포 작업 모습] */}
...
{/* [이미지 8: 밤늦게 코딩하다가 "Build Successful" 메시지를 보고 기뻐하는 개발자] */}
...
{/* [이미지 9: 자신감 있는 표정으로 코드 리뷰하는 개발자, 성장한 모습] */}
